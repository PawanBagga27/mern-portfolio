                ...
                <div className='flex flex-col gap-10 border-l-2 border-[#135e4c82] w-1/4 sm:flex-row sm:overflow-scroll sm:w-full'>
                    {experiences.map((experiences, index) => (
                        <div onClick={() => {
                            ...
                            <h1 className={`text-xl px-5 ${selectedItemIndex === index ? 'text-tertiary border-tertiary border-l-4 -ml-[3px] bg-[#1a7f5a31] p-3 sm:w-full' : 'text-white'}`}>{experiences.period}</h1>
                        </div>
                    ))}
                </div>
